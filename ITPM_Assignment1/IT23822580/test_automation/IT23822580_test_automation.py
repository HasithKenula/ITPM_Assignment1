...
DEFAULT_EXCEL_CANDIDATES = [
    str(TESTS_DIR / "IT23822580_Test cases.xlsx"),
]
...
def _resolve_path(p: str | None) -> str | None:
    ...
    if not p:
        return None
    # Tolerate accidental spaces after path separators, e.g. "test_automation/ file.xlsx"
    p = re.sub(r"([\\/])\s+", r"\1", p)
    path = Path(p)
    if path.is_absolute():
        return str(path)
...
def _find_column_index(header_values: list, requested_name: str | None, candidates: list[str]) -> int | None:
    ...
    def match_exact(name: str) -> int | None:
    ...
    def match_fuzzy(name: str) -> int | None:
        n = _normalize_header(name)
        if not n:
            return None
        for i, v in indexed:
    ...
    if requested_name:
        found = match_exact(requested_name)
        if found:
            return found
        found = match_fuzzy(requested_name)
        if found:
            return found

    # Prefer exact candidate matches first to avoid selecting broad fuzzy matches
    # like "Singlish" when "Input" exists as a dedicated column.
    for c in candidates:
        found = match_exact(c)
        if found:
            return found
